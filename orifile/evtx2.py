    ...
    with evtx.Evtx(EvtxPath) as log:
        for record in log.records():
            ...
            if EventID == "4624":
                ...
                dataInsideList['SystemTime']=time
                dataInsideList['EventID']=EventID
                dataInsideList['Computer']=Computer
                for data in Datas:
                    dataInsideList['TargetUserName_EventData']=data.get("TargetUserName_EventData")
                    dataInsideList['IpAddress_EventData']=data.get("IPAddress")
                dataList.append(dataInsideList)
                print(dataList)
                # 输出遍历数据
                # for data in Datas:
